...
#Создаем парсер параметров запуска, чтобы иметь возможность выбрать модель и номер части данных
...
with ThreadPoolExecutor(8) as executor:
    ...
    for i in executor.map(get_recos, users):
        finished += 1
        print("COMPUTING: {}/{}".format(finished, num_users), end="\r", flush=True)

tf = time.time() - ts

print("Offline recommendations have been computed for {} seconds".format(round(tf)))
